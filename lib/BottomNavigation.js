...
export default class BottomNavigation extends Component {
  ...
  // FIXME: componentDidUpdate will be called multiple times, like 3 times or so
  componentDidUpdate() {
    setTimeout(() => this._measure(), 0)
  }
  ...
  componentWillReceiveProps(nextProps: Props) {
    ...
    if (newTabIndex !== oldTabIndex) {
      ...
      if (newTabIndex >= 0 && newTabIndex < tabAmount) {
        ...
        if (__DEV__) {
          console.error(`${newTabIndex} is not a valid tabIndex`)
        }
      }
    }
  }
  ...
  _handleTabChange = ({ tabIndex, barBackgroundColor, iconRef }) => {
  ...
  _measure = () => {
    ...
    this.props.children.forEach((child, i) => {
      this.refs[`tab_${i}`].getIconRef().measureLayout(navHandle, (x, y) => {
        this.iconPositions[i] = { x, y }
      })
    })
  }
}
